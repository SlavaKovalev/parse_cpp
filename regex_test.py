...
def trace_at_begin(allfile, item, reobj, what_to_print):
  ...
  nsrt = "TRACE(L\"%d {}\\n\", GetCurrentThreadId());".format(what_to_print)
  tl = allfile[item.end():]
  newallfile = bfr
  newallfile += nsrt
  newallfile += tl
...
def add_trace():
  reobj = re.compile("^[ ]*[^\n()]+[ ]+[^\n()]+[::]*[a-zA-Z]+\([^;{]*\n[ ]*{", flags=re.MULTILINE)
  for  root, dirs,files in os.walk('C:\\Users\\Svyatoslav.Kovalev\\Documents\\tib-mounter-threadpool\\tib-mounter\\kernel\\win\\tib_mounter\\service'):
      for file in files:
        try:
          if ".cpp" in file:
            pos = 0
